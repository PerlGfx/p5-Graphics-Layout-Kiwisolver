...
fun tree($solver, $n, $leaves) {
	my $node = Renard::API::Kiwisolver::Variable->new;
	...
	if( $n > 1 ) {
		...
		$solver->addEditVariable($node, Renard::API::Kiwisolver::Strength::WEAK );
...
subtest "Tree benchmark" => fun() {
	...
	$solver->addEditVariable($root, Renard::API::Kiwisolver::Strength::STRONG );
